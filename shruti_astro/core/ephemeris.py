...
def _rise_or_set(jd_start: float, lat: float, lon: float, rsmi: int) -> datetime | None:
    """
    ...
    No BIT_DISC_CENTER and no BIT_NO_REFRACTION: the traditional definition uses
    the visible disc with refraction, which is also what every almanac prints.
    (BIT_HINDU_RISING exists for the Indian convention — centre of disc, no
    refraction — and is the right flag if a pañcāṅga surface ever needs sunrise
    to match an Indian almanac exactly.)
    """
    _ensure_init()
    res, tret = swe.rise_trans(
        jd_start, swe.SUN, rsmi, (lon, lat, 0.0), 0.0, 0.0, _flags()
    )
...
def sun_events(day: datetime, lat: float, lon: float) -> tuple[datetime, datetime, datetime]:
    """
    ...
    sunrise = _rise_or_set(jd0, lat, lon, swe.CALC_RISE)
    if sunrise is None:
        raise SunNeverRose("the Sun did not rise at this location on this date")

    sunset = _rise_or_set(_julday(sunrise), lat, lon, swe.CALC_SET)
    if sunset is None:
        raise SunNeverRose("the Sun did not set at this location on this date")

    next_sunrise = _rise_or_set(_julday(sunset), lat, lon, swe.CALC_RISE)
